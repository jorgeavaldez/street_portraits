defmodule StreetPortraitsWeb.AccessKeyControllerTest do
  ...
  describe "index" do
    test "lists all access_keys", %{conn: conn} do
      conn = get(conn, Routes.access_key_path(conn, :index))
  ...
  describe "new access_key" do
    test "renders form", %{conn: conn} do
      conn = get(conn, Routes.access_key_path(conn, :new))
  ...
  describe "create access_key" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post(conn, Routes.access_key_path(conn, :create), access_key: @create_attrs)

      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == Routes.access_key_path(conn, :show, id)

      conn = get(conn, Routes.access_key_path(conn, :show, id))
      assert html_response(conn, 200) =~ "Show Access key"
    end

    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, Routes.access_key_path(conn, :create), access_key: @invalid_attrs)
      assert html_response(conn, 200) =~ "New Access key"
    end
  end
  ...
  describe "edit access_key" do
    ...
    test "renders form for editing chosen access_key", %{conn: conn, access_key: access_key} do
      conn = get(conn, Routes.access_key_path(conn, :edit, access_key))
  ...
  describe "update access_key" do
    ...
    test "redirects when data is valid", %{conn: conn, access_key: access_key} do
      conn = put(conn, Routes.access_key_path(conn, :update, access_key), access_key: @update_attrs)
      assert redirected_to(conn) == Routes.access_key_path(conn, :show, access_key)

      conn = get(conn, Routes.access_key_path(conn, :show, access_key))
    ...
    test "renders errors when data is invalid", %{conn: conn, access_key: access_key} do
      conn = put(conn, Routes.access_key_path(conn, :update, access_key), access_key: @invalid_attrs)
      assert html_response(conn, 200) =~ "Edit Access key"
    end
  end
  ...
  describe "delete access_key" do
    ...
    test "deletes chosen access_key", %{conn: conn, access_key: access_key} do
      conn = delete(conn, Routes.access_key_path(conn, :delete, access_key))
      assert redirected_to(conn) == Routes.access_key_path(conn, :index)
      assert_error_sent 404, fn ->
        get(conn, Routes.access_key_path(conn, :show, access_key))
      end
    end
  end
